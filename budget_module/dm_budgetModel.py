...
def prepare_single_job_budget_model():
...
def create_model_cross_val(data_frame, label_name, is_classification):
    ...
    model.fit(data_frame, data_frame_target)
    return model, data_frame.columns, predictions, data_frame_target
...
def budget_model():
    ...
    if not do_cross_val:
        ...
        # model = create_model(df_train.copy(), label_name, budget_classification, selectbest=False)
        # print_model_evaluation(model, df_test_outl.copy(), label_name, budget_classification)
        ...
        df_train_outl, df_test_outl = normalize_test_train(df_train_outl,
                                                           df_test_outl,
                                                           label_name=label_name,
                                                           z_score_norm=False,
                                                           weighting=True)
        model, columns = create_model(df_train_outl, label_name,
                                      budget_classification, selectbest=False)
        # print_model_evaluation(model, df_test_outl, label_name, budget_classification)
        ...
        model, columns, predictions, data_frame_target =\
            create_model_cross_val(df_outl, label_name, budget_classification)

        # print_predictions_comparison(data_frame_target,
        #                              predictions, label_name, 20)
